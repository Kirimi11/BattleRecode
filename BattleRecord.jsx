...
import { Plus, Trash2, Swords, TrendingUp, X, Trophy, Camera, Loader2, Pencil, Calendar as CalendarIcon, ChevronLeft, ChevronRight, Sparkles, BookOpen, MessageCircle, Home, Send, ExternalLink, Award, ListTree, Download, Link as LinkIcon, Upload, Database, Delete, ArrowRight, Check, PieChart as PieChartIcon, ImagePlus } from "lucide-react";
...
// Chat call: multi-turn text conversation with a system instruction.
async function callGeminiChat(apiKey, systemPrompt, messages, maxTokens = 600) {
...
const REMEMBERED_KEY = "remembered_gate_password";
...
// deterministic (non-secret) identifier derived from a password, used only to name the
// storage slot for that password's vault - the actual data is protected by AES-GCM above
...
export default function BattleRecord() {
  ...
  const [gateTextMode, setGateTextMode] = useState(false);
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await window.storage.get(REMEMBERED_KEY, false);
        if (res) {
          const saved = JSON.parse(res.value);
          if (saved && typeof saved.password === "string") {
            setCheckingRemembered(false);
            await enterApp(saved.password);
            return;
          }
        }
      } catch {
        // fall through to showing the normal gate screen
      }
      setCheckingRemembered(false);
    })();
  ...
  const saveVaultSnapshot = async (overrides) => {
    if (!vaultActive || !vaultKey || !vaultPassword) return;
    const payload = {
      matches: overrides.matches ?? matches,
      tournamentPhotos: overrides.tournamentPhotos ?? tournamentPhotos,
      tournamentPoints: overrides.tournamentPoints ?? tournamentPoints,
      deckProfiles: overrides.deckProfiles ?? deckProfiles,
      links: overrides.links ?? links,
      externalMetaShares: overrides.externalMetaShares ?? externalMetaShares,
    };
    try {
      const encrypted = await encryptJSON(payload, vaultPassword);
      await window.storage.set(vaultKey, JSON.stringify(encrypted), false);
    } catch {
  ...
  const skipGate = async () => {
    ...
    await initNormalBoards();
  };

  const forgetDevice = async () => {
    try {
      const res = await window.storage.get(REMEMBERED_KEY, false);
      if (res) await window.storage.delete(REMEMBERED_KEY, res.version);
      setImportMessage("この端末の自動ログインを解除しました。次回開いたときはパスワード入力が必要です。");
    } catch {
      setError("解除に失敗しました。もう一度お試しください。");
    }
  };

  const enterApp = async (overridePassword) => {
    const raw = overridePassword !== undefined ? overridePassword : gatePassword;
    setGateError(null);
    if (!raw.trim()) {
      setVaultActive(false);
      setGateResolved(true);
    ...
    try {
      ...
      const vKey = `vault_${hash.slice(0, 24)}`;
      let payload = {
        matches: [],
        tournamentPhotos: {},
        tournamentPoints: [],
      ...
      const res = await window.storage.get(vKey, false).catch(() => null);
      if (res) {
        const encryptedObj = JSON.parse(res.value);
        payload = await decryptJSON(encryptedObj, pw);
      }
      setMatches(payload.matches || []);
      setTournamentPhotos(payload.tournamentPhotos || {});
      setTournamentPoints(payload.tournamentPoints || []);
      setDeckProfiles(payload.deckProfiles || []);
      setLinks(payload.links || []);
      setExternalMetaShares(payload.externalMetaShares || []);
      setBoards([{ id: DEFAULT_BOARD_ID, name: "メイン" }]);
      ...
      if (rememberDevice) {
        try {
          await window.storage.set(REMEMBERED_KEY, JSON.stringify({ password: pw }), false);
        } catch {
          // remembering is best-effort; unlocking has already succeeded
        }
      }
    } catch {
      setGateError("データの読み込みに失敗しました。パスワードをご確認ください。");
    } finally {
  ...
  const persist = async (next) => {
    setMatches(next);
    if (vaultActive) {
      await saveVaultSnapshot({ matches: next });
      return;
    }
    try {
  ...
  const persistPhotos = async (next) => {
    ...
    if (vaultActive) {
      await saveVaultSnapshot({ tournamentPhotos: next });
      return;
    }
    try {
  ...
  const savePhoto = (tKey, dataUrl) => {
    ...
    const next = {
      ...tournamentPhotos,
      [tKey]: [...(tournamentPhotos[tKey] || []), photo],
    };
    persistPhotos(next);
  };
  ...
  const deletePhoto = (tKey, id) => {
    const next = {
      ...tournamentPhotos,
      [tKey]: (tournamentPhotos[tKey] || []).filter((p) => p.id !== id),
    };
    persistPhotos(next);
  };
  ...
  const persistExternalMeta = async (next) => {
    ...
    if (vaultActive) {
      await saveVaultSnapshot({ externalMetaShares: next });
      return;
    }
    try {
      await window.storage.set(
      ...
      setError("メタデータの保存に失敗しました。もう一度お試しください。");
    }
  };
  ...
  const handleMetaImageUpload = async (e) => {
    ...
    try {
      ...
      setMetaOcrError("画像の読み取りに失敗しました。もう一度お試しください。");
    } finally {
  ...
  const sendChatMessage = async () => {
    ...
    if (!geminiApiKey) {
      setChatMessages([
        ...
        {
          role: "assistant",
          text: "Gemini APIキーが未設定です。データ管理画面で登録してください。",
        },
      ]);
      return;
    }
    ...
    try {
      const context = {
        総合成績: stats.overall,
        デッキ相性: matchupAnalysis.map((d) => ({
          自分のデッキ: d.myDeck,
          総合勝率: Math.round(d.rate),
          対戦相手デッキ別: d.rows.map((r) => ({
            相手デッキ: r.opponentDeck,
            勝率: Math.round(r.rate),
            試合数: r.total,
          })),
        })),
        最近よく対戦する相手デッキ: recentOpponentFreq,
      };
      const systemPrompt =
        "あなたはカードゲームの対戦データ分析アシスタントです。以下のJSONはユーザー自身の対戦記録から集計した相性データです。" +
        "これを根拠にユーザーの質問（次に使うデッキの相談など）に、具体的なデッキ名と数値を挙げて簡潔な日本語で答えてください。" +
        "データにない情報は推測で断定せず、その旨を伝えてください。\n\nデータ:\n" +
        JSON.stringify(context);

      const textBlock = await callGeminiChat(geminiApiKey, systemPrompt, nextMessages, 600);
      setChatMessages([
        ...nextMessages,
        { role: "assistant", text: textBlock || "回答を取得できませんでした。" },
      ]);
    } catch {
      setChatMessages([
        ...nextMessages,
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      setOcrError("画像の読み取りに失敗しました。もう一度お試しください。");
    } finally {
      setOcrLoading(false);
    }
  };
    ...
    return Object.entries(byPeriod)
      .map(([periodKey, entries]) => {
        ...
        const weeks = Object.entries(byWeek)
          ...
          .map(([weekKey, weekEntries], idx) => {
            ...
            return {
              weekIndex: idx + 1,
              weekKey,
              reflectedPoints: sortedWeek.filter((e) => top2Ids.has(e.id)).map((e) => e.points),
              excludedPoints: sortedWeek.filter((e) => !top2Ids.has(e.id)).map((e) => e.points),
            };
          });
        ...
        return {
          periodKey,
          entries: sorted,
          reflected: top2.reduce((s, e) => s + e.points, 0),
          excluded: rest.reduce((s, e) => s + e.points, 0),
          weeks,
          winnerPromo,
          best8Promo,
        };
  ...
  const periodTop2Ids = useMemo(() => {
    const map = {};
    periodPoints.forEach((p) => {
      map[p.periodKey] = new Set(p.entries.slice(0, 2).map((e) => e.id));
    });
    return map;
  }, [periodPoints]);
  ...
  const recommendation = useMemo(() => {
    ...
    const myDeckNames =
      matchupAnalysis.length > 0 ? matchupAnalysis.map((d) => d.myDeck) : recentMyDecks;
  ...
  }, [matchupAnalysis, recentOpponentFreq, deckCategoryMap, recentMyDecks]);
  ...
  // recent unique tournament names, for quick-select chips in the form
  ...
  if (checkingRemembered) {
    return (
      <div style={styles.gatePinPage}>
        <style>{fontImport}</style>
        <Loader2 size={22} className="spin" color="rgba(242,239,232,0.6)" />
      </div>
    );
  }
  ...
  if (!gateResolved) {
    const maxPinLen = 6;
    const pinDigits = gatePassword.replace(/\D/g, "").slice(0, maxPinLen);
    const keypadRows = [
      ["1", "2", "3"],
      ["4", "5", "6"],
      ["7", "8", "9"],
    ...
    const tapDigit = (d) => {
      if (gateTextMode || gateLoading) return;
      if (pinDigits.length >= maxPinLen) return;
      const next = pinDigits + d;
      setGatePassword(next);
      if (next.length === maxPinLen) {
        setTimeout(() => enterApp(), 80);
      }
    };
    const tapBackspace = () => {
      if (gateTextMode || gateLoading) return;
      setGatePassword(pinDigits.slice(0, -1));
    };
    ...
    return (
      <div style={styles.gatePinPage}>
        ...
        <div style={styles.gatePinTop}>
          <span style={styles.gatePinTitle}>戦績帳</span>
          <p style={styles.gatePinDesc}>
            {gateTextMode
              ? "パスワードを入力してください。未入力なら通常のデータで開始します。"
              : "数字を入力するとそのパスワードのデータを開きます。空のまま右下のボタンで通常のデータを開けます。"}
          </p>
        </div>
        ...
        {gateTextMode ? (
          <div style={styles.gateTextBox}>
            <input
              ...
              autoFocus
              onKeyDown={(e) => {
                if (e.key === "Enter" && !gateLoading) enterApp();
              }}
            />
            <label style={styles.gateRememberRow}>
              <input
                type="checkbox"
            ...
            {gateError && <div style={styles.gatePinError}>{gateError}</div>}
            <button onClick={enterApp} disabled={gateLoading} style={styles.submitBtn}>
              {gateLoading ? (
          ...
          <>
            <div style={styles.gatePinDots}>
              {Array.from({ length: maxPinLen }).map((_, i) => (
                <span
                  key={i}
                  style={{
                    ...styles.gatePinDot,
                    ...(i < pinDigits.length ? styles.gatePinDotFilled : {}),
                  }}
                />
              ))}
            </div>
            ...
            <div style={styles.gatePinGrid}>
              ...
              <button
                ...
                aria-label="パスワードなしで開始"
                disabled={gateLoading}
              >
                {gateLoading ? <Loader2 size={20} className="spin" /> : <ArrowRight size={22} />}
              </button>
              <button onClick={() => tapDigit("0")} style={styles.gatePinKey}>
                0
              </button>
              <button onClick={tapBackspace} style={styles.gatePinKeyIcon} aria-label="1文字削除">
                <Delete size={20} />
              </button>
            </div>
  ...
  return (
    <div style={styles.page}>
      ...
      <header style={styles.header}>
        <div style={styles.headerRow}>
          ...
          <button
            ...
            style={styles.calendarToggleBtn}
            aria-label="デッキ相談チャット"
          >
            <MessageCircle size={16} color={chatOpen ? "#fdfaf4" : "#8a7a5c"} />
          </button>
          <a
      ...
      {chatOpen && (
        <section style={styles.calendarCard}>
          <div style={styles.deckDbHeader}>
            <span style={styles.deckDbTitle}>デッキ相談チャット</span>
          </div>
          <div style={styles.chatMessages}>
            {chatMessages.length === 0 && (
              <div style={styles.empty}>
                「明日は何を使うべき？」「◯◯デッキとの相性は？」のように聞いてみてください。記録した対戦データをもとに答えます。
              </div>
            )}
            ...
            {chatLoading && (
              <div style={styles.chatBubbleAssistant}>
                <Loader2 size={14} className="spin" /> 考え中…
              </div>
            )}
          </div>
      ...
      {dataManageOpen && (
        <section style={styles.calendarCard}>
          ...
          {vaultActive && (
            <>
              <div style={styles.deckDbHeader}>
                <span style={styles.deckDbTitle}>この端末の自動ログイン</span>
              </div>
              <div style={styles.hintText}>
                パスワード入力画面で「この端末に保存して次回から自動的に開く」を選んでいる場合、この端末では次回から入力なしでこのデータが開きます。
              </div>
              <button onClick={forgetDevice} style={styles.boardDeleteBtn}>
                この端末の自動ログインを解除する
              </button>
            </>
          )}
            ...
            <ResponsiveContainer width="100%" height={190}>
              <PieChart>
                ...
                <Tooltip
                  ...
                  formatter={(value, name) => [`${value}戦`, name]}
                />
              </PieChart>
            </ResponsiveContainer>
          </div>
      ...
      {(stats.byOpponent.length > 0 || stats.byOpponentDeck.length > 0 || stats.byMyDeck.length > 0 || stats.byTournament.length > 0 || stats.byTurnOrder.length > 0 || matchupAnalysis.length > 0) && (
        <section style={styles.breakdownSection}>
          ...
          {filter === "tournament" && filterValue && (
            <div style={styles.gallerySection}>
              <button
                onClick={() => {
                  const row = stats.byTournament.find((r) => r.key === filterValue);
                  if (row) openAddToTournament(row);
                }}
              ...
              <div style={styles.galleryLabel}>この大会の写真</div>
              <div style={styles.gallery}>
                {(tournamentPhotos[filterValue] || []).map((p) => (
                  <div key={p.id} style={styles.galleryItem}>
                    <img
                      src={p.dataUrl}
                      alt="大会の写真"
                      style={styles.galleryImg}
                      onClick={() => setViewingPhoto({ kind: "tournament", tKey: filterValue, ...p })}
                    />
                  </div>
                ))}
                <label style={styles.galleryAddTile}>
                  <input
                    ...
                    onChange={(e) => handleAddPhotoToTournament(e, filterValue)}
                  />
                  {photoAddLoading ? (
      ...
      <section style={styles.logSection}>
        <div style={styles.logHeader}>
          ...
          <span style={styles.logCount}>{filtered.length}件</span>
        </div>
                    ...
                    <div key={r.id} style={styles.batchRound}>
                      ...
                      {recentOpponentDecks.length > 0 && (
                        <div style={styles.chipRowTight}>
                          {recentOpponentDecks.slice(0, 5).map((d) => (
                            <button
                              key={d}
